...
from clustering import compute_clusters_sequence, compute_clusters_structure
...
parser.add_argument('--scratch', type=str, help='Path to scratch (if on cluster).', default='$SCRATCH/proteinshake')
...
if os.path.exists(f'{SCRATCH}/{DATASET}'):
    pass#shutil.rmtree(f'{SCRATCH}/{DATASET}')
ds = Dataset(**kwargs)
