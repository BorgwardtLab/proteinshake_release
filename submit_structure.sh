...
for DATASET in "${datasets[@]}"; do
        if [[ $DATASET != "AlphaFoldDataset" && $DATASET != "RCSBDataset" ]]; then
            ...
            JOBID=$(sbatch --parsable --ntasks 1 --cpus-per-task 2 --mem-per-cpu 10G --time 3:59:00 -o $LOGDIR/${DATASET}_structure_compute.log -J $DATASET --array 0-$NUM_TASKS%$MAX_TASKS --wrap "python -m cluster_structure --compute --njobs 1 --dataset $DATASET")
